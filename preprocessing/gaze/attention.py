...
def compute_attention_signals(recordings, min_timestamp, max_timestamp, bin_width_sec=1):
    ...
    data_table = data_table[['start timestamp [sec]', 'end timestamp [sec]', 'mapped_aoi']]
    ...
    time_series = {c: list() for c in data_table['mapped_aoi'].unique()}
    ...
    for start_sec in np.arange(min_timestamp, max_timestamp, bin_width_sec):
        ...
        out = df_copy.groupby('mapped_aoi').agg("sum")
...
if __name__ == '__main__':
    ...
    with open(args.meta, 'r+') as f:
        ...
        meta['artifacts']['time']['attention'] = str(out_path)
